...
{ 
  ...
  double closest_distance_km = 1000; /* over 1000KM and I don't even want an answer. */
  double this_distance_km;
  double searchlat; 
  double searchlon; 
  int ret;
  ...
  { 
    ...
    { 
      ...
      this_distance_km = sqrt(
        pow( 110.574 * (atof(current_city.Latitude) - searchlat), 2) + 
        pow( 111.320 * (atof(current_city.Longitude) - searchlon) * (cos(searchlat * 0.017453293)), 2)
      );

      if (this_distance_km < closest_distance_km) 
      { 
        closest_distance_km = this_distance_km;
        closest_city = current_city;
#ifdef DEBUG
        printf("New closest city at %f: ", closest_distance_km);
        print_city(&closest_city); 
#endif 
      }
      /* we can quit early if we provably move farther away in longitude than the closest distance_km. */
...
{ 
  ...
  { 
    printf ("Usage:  %s logfile gridsquare\n", argv[0]); 
    exit (1); 
  }
